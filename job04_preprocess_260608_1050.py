...
# 2. 데이터 로드 및 기본 확인
# ------------------------------------------------------------------------------------------------
df = pd.read_csv('./data/news_titles_260608_1050.csv')
...
with open('./data/encoder_260608_1050.pkl', 'wb') as f:     # 라벨 이름 저장
...
for i in range(len(X)):
    X[i] = re.sub('[^가-힣]', ' ', X[i])         # 한글을 제외한 문자는 공백으로 변경
...
for sentence in tokened_x:
    if max < len(sentence):
        max = len(sentence)
print(max)
with open('./data/tokenizer_max{}_260608_1050.pkl'.format(max), 'wb') as f:
...
# 10. 전처리 데이터 저장
# ------------------------------------------------------------------------------------------------
np.save('./data/x_train_wordsize{}_260608_1050.npy'.format(wordsize), x_train)
np.save('./data/y_train_wordsize{}_260608_1050.npy'.format(wordsize), y_train)
np.save('./data/x_test_wordsize{}_260608_1050.npy'.format(wordsize), x_test)
np.save('./data/y_test_wordsize{}_260608_1050.npy'.format(wordsize), y_test)